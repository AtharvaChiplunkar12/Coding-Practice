...
Topics - Array, Dynamic Programming, Greedy, Bit Manipulation, Tree, Sorting
            (I only solved it with greedy)
            
There exists an undirected tree with n nodes numbered 0 to n - 1. You are given a 0-indexed 
2D integer array edges of length n - 1, where edges[i] = [ui, vi] indicates that there is an edge 
...
- Choose the edge [0,2]. nums[0] and nums[2] become: 1 XOR 3 = 2, and the array nums becomes: 
[1,2,1] -> [2,2,2].
...
- Choose the edge [0,1]. nums[0] becomes: 2 XOR 7 = 5 and nums[1] become: 3 XOR 7 = 4, and 
the array nums becomes: [2,3] -> [5,4].
